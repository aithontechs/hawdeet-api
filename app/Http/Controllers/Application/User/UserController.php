...
class UserController extends Controller
{
    ...
    public function profile()
    {
        ...
        $user->load([
            'subscriptions' => fn ($q) =>
                $q->where('status' , 'active')->where('payment_status' , 'paid')->select(['id','user_id','start_at','end_at','price','status','payment_status','canceled_at','ended_reason' , 'plan_id'])
    ...
    public function updateProfile(UserUpdateRequest $request)
    {
        ...
        if (isset($data['phone'])) {
            if (is_null($user->phone)) {
                ...
                $data['preferred_currency'] = $this->phoneCurrencyService->resolveFromPhoneOrDefault($data['phone']);
            } else {
                unset($data['phone'], $data['preferred_currency']);
            }
        }
        $user->update($data);
